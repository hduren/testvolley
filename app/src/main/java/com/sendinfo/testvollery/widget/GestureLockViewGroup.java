...
import com.sendinfo.testvollery.R;
import com.sendinfo.testvollery.widget.GestureLockView.Mode;
...
//                }
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                //设置参数，主要是定位GestureLockView间的位置
                LayoutParams lockerParams =(LayoutParams) getChildAt(i).getLayoutParams();


                lockerParams.width=mGestureLockViewWidth;
                lockerParams.height=mGestureLockViewWidth;
                //设置右下左上的边距
                int rightMargin = mMarginBetweenLockView;
                int bottomMargin = mMarginBetweenLockView;
                int leftMagin = 0;
...
//
                lockerParams.setMargins(leftMagin, topMargin, rightMargin,
                        bottomMargin);
                Log.e("margin", leftMagin + "  " + topMargin + "  " + rightMargin + " " + bottomMargin);

                mGestureLockViews[i].setMode(Mode.STATUS_NO_FINGER);
                mGestureLockViews[i].setLayoutParams(lockerParams);
                measureChild(getChildAt(i), mGestureLockViewWidth, mGestureLockViewWidth);
...
//
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Log.e("onlayout",l+" t"+t+" r"+r+"  b"+b);
        final int count = getChildCount();
        // These are the far left and right edges in which we are performing layout.
        int leftPos = getPaddingLeft();
        int rightPos = r - l - getPaddingRight();
        int topPos=getPaddingTop();
//        int buttonPos=b-t-getPaddingBottom();
        int left=leftPos,right=0,top=topPos,botton=0;
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if(left+lp.leftMargin+lp.rightMargin+mGestureLockViewWidth>rightPos){
                    left=leftPos;
                    top+=mGestureLockViewWidth;
                    top+=lp.bottomMargin;
                }

                left+=lp.leftMargin;
//                top+=lp.topMargin;
                child.layout(left,top,left+mGestureLockViewWidth,top+mGestureLockViewWidth);
                left+=lp.rightMargin+mGestureLockViewWidth;
//
